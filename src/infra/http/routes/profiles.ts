...
export async function profileRoutes(app: FastifyInstance) {
  app.get('/profiles/me', { preHandler: authGuard }, async (req) => {
    ...
    try {
      const { rows: testRows } = await pool.query(
        `SELECT up.profile_data, up.health_metrics, up.age, up.gender, up.height_cm, up.weight_kg, u.username, u.email
         FROM users u
  ...
  app.post('/profiles/save', { preHandler: authGuard }, async (req, reply) => {
    ...
    try {
      ...
      try {
        await client.query(
          `INSERT INTO user_profiles(user_id, profile_data, health_metrics, age, gender, height_cm, weight_kg, updated_at)
           ...
           ON CONFLICT (user_id) DO UPDATE SET 
             ...
             updated_at = now()`,
          [user.userId, body.profile, body.metrics, age, gender, height_cm, weight_kg]
        );
      } catch (e: any) {
        // If columns don't exist, use fallback query without biometric columns
        if (e.code === '42703') {
