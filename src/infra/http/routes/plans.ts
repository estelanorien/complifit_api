...
export async function plansRoutes(app: FastifyInstance) {
      ...
      for (let i = 0; i < nutrition.days.length; i++) {
        ...
        if (Array.isArray(day.meals)) {
          for (const meal of day.meals) {
            await client.query(
              `INSERT INTO meals(id, meal_plan_day_id, type, name, calories, macros, time_label, ingredients, instructions, metadata, created_at)
               VALUES(gen_random_uuid(), $1, $2, $3, $4, $5::jsonb, $6, $7::jsonb, $8::jsonb, $9::jsonb, now())`,
              [
                dayId,
                meal.type || 'meal',
  ...
  app.post('/plans/save-generated', { preHandler: authGuard }, async (req, reply) => {
    ...
    try {
      ...
      console.error("save-generated failed", e);
  ...
  app.post('/plans/reroll/meal', { preHandler: authGuard }, async (req, reply) => {
    ...
    Language: ${body.lang}.
    Return JSON object (Meal structure): { "type": "...", "recipe": { "name": "...", "calories": number, "time": "string", "ingredients": [], "instructions": [] } }
    `;
